...
const RoleView = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    api
      .get(`/auth/role?page=${currentPage}&pageSize=${pageSize}`)
      .then((result) => {
        // console.log(result.data); // Check API response in the console
        if (result.data.roles) {
          setrole(result.data.roles); // Set batch data
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this role record?")) {
      api
        ...
        .then((result) => {
          if (result.data) {
            setrole(role.filter((item) => item.id !== id)); // Remove deleted batch from state
  ...
  return (
    <div className="px-5 mt-3">
      <div className="d-flex justify-content-center">
        <h3 style={{ color: "red" }}>List of role Shown Below:</h3>
      </div>
      {permissions.canAdd && (
        <Link to="/dashboard/role/add" className="btn btn-success">
          Add role
        </Link>
      )}
