...
const CourseView = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    api
      .get(`/auth/course?page=${currentPage}&pageSize=${pageSize}`)
      .then((result) => {
        // console.log(result.data); // Check API response in the console
        if (result.data.courses) {
          setCourses(result.data.courses); // Set batch data
  ...
  const handleEdit = (id) => {
    navigate(`/dashboard/course/edit/${id}`);
  };
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this course record?")) {
      api
        .delete(`/auth/course/${id}`)
        .then((result) => {
          if (result.data) {
            setCourses(courses.filter((item) => item.id !== id)); // Remove deleted batch from state
  ...
  return (
    <div className="px-5 mt-3">
      <div className="d-flex justify-content-center">
        <h3 style={{ color: "red" }}>List of Course Shown Below:</h3>
      </div>
      {permissions.canAdd && (
        <Link to="/dashboard/course/add" className="btn btn-success">
          Add Course
        </Link>
      )}
                ...
                {Array.isArray(courses) && courses.length > 0 ? (
                  courses.map((c) => (
                    <TableRow key={c.id}>
                      ...
                      <TableCell>
                        <Typography fontWeight="bold">{c.Sname}</Typography>
                      </TableCell>
                      <TableCell>
