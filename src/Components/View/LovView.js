...
const StatusView = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    api
      .get(`/auth/status?page=${currentPage}&pageSize=${pageSize}`)
      .then((result) => {
        // console.log(result.data); // Check API response in the console
        if (result.data.statuses) {
          setStatus(result.data.statuses); // Set batch data
  ...
  const handleEdit = (id) => {
    navigate(`/dashboard/lov/edit/${id}`);
  };
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this Lov record?")) {
      api
        .delete(`/auth/status/${id}`)
        .then((result) => {
          if (result.data) {
            setStatus(status.filter((item) => item.id !== id)); // Remove deleted batch from state
  ...
  return (
    <div className="px-5 mt-3">
      <div className="d-flex justify-content-center">
        <h3 style={{ color: "red" }}>List of Lov Shown Below:-</h3>
      </div>
      {permissions.canAdd && (
        <Link to="/dashboard/lov/add" className="btn btn-success">
          Add Lov
        </Link>
      )}
