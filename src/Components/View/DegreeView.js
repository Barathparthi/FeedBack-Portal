...
const DegreeView = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    api
      .get(`/auth/degree?page=${currentPage}&pageSize=${pageSize}`)
      .then((result) => {
        // console.log(result.data); // Check API response in the console
        if (result.data.degrees) {
          setDegree(result.data.degrees); // Set batch data
  ...
  const handleEdit = (id) => {
    navigate(`/dashboard/degree/edit/${id}`);
  };
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this degree record?")) {
      api
        .delete(`/auth/degree/${id}`)
        .then((result) => {
          if (result.data) {
            setDegree(degree.filter((item) => item.id !== id)); // Remove deleted batch from state
  ...
  return (
    <div className="px-5 mt-3">
      <div className="d-flex justify-content-center">
        <h3 style={{ color: "red" }}>List of Degree Shown Below:</h3>
      </div>
      {permissions.canAdd && (
        <Link to="/dashboard/degree/add" className="btn btn-success">
          Add Degree
        </Link>
      )}
