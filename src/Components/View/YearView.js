...
const YearView = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    api
      .get(`/auth/year?page=${currentPage}&pageSize=${pageSize}`)
      .then((result) => {
        // console.log(result.data); // Check API response in the console
        if (result.data.years) {
          setYear(result.data.years); // Set batch data
  ...
  const handleEdit = (id) => {
    navigate(`/dashboard/year/edit/${id}`);
  };
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this year record?")) {
      api
        .delete(`/auth/year/${id}`)
        .then((result) => {
          if (result.data) {
            setYear(year.filter((item) => item.id !== id)); // Remove deleted batch from state
  ...
  return (
    <div className="px-5 mt-3">
      <div className="d-flex justify-content-center">
        <h3 style={{ color: "red" }}>List of Year Shown Below:-</h3>
      </div>
      {permissions.canAdd && (
        <Link to="/dashboard/year/add" className="btn btn-success">
          Add Year
        </Link>
      )}
