...
const RegulationView = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    api
      .get(`/auth/regulation?page=${currentPage}&pageSize=${pageSize}`)
      .then((result) => {
        // console.log(result.data); // Check API response in the console
        if (result.data.regulations) {
          setregulation(result.data.regulations); // Set batch data
  ...
  const handleEdit = (id) => {
    navigate(`/dashboard/regulation/edit/${id}`);
  };
  ...
  const handleDelete = (id) => {
    if (
      window.confirm("Are you sure you want to delete this regulation record?")
    ) {
      api
        .delete(`/auth/regulation/${id}`)
        .then((result) => {
          if (result.data) {
            setregulation(regulation.filter((item) => item.id !== id)); // Remove deleted batch from state
  ...
  return (
    <div className="px-5 mt-3">
      <div className="d-flex justify-content-center">
        <h3 style={{ color: "red" }}>List of Regulation Shown Below:</h3>
      </div>
      {permissions.canAdd && (
        <Link to="/dashboard/regulation/add" className="btn btn-success">
          Add regulation
        </Link>
      )}
