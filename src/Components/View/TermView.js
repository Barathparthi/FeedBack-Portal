...
const TermView = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    api
      .get(`/auth/term?page=${currentPage}&pageSize=${pageSize}`)
      .then((result) => {
        // console.log(result.data); // Check API response in the console
        if (result.data.terms) {
          setTerm(result.data.terms); // Set batch data
  ...
  const handleEdit = (id) => {
    navigate(`/dashboard/term/edit/${id}`);
  };
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this term record?")) {
      api
        .delete(`/auth/term/${id}`)
        .then((result) => {
          if (result.data) {
            setTerm(term.filter((item) => item.id !== id)); // Remove deleted batch from state
  ...
  return (
    <div className="px-5 mt-3">
      <div className="d-flex justify-content-center">
        <h3 style={{ color: "red" }}>List of Term Shown Below:-</h3>
      </div>
      {permissions.canAdd && (
        <Link to="/dashboard/term/add" className="btn btn-success">
          Add Term
        </Link>
      )}
