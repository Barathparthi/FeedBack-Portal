...
const SemesterView = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    api
      .get(`/auth/semester?page=${currentPage}&pageSize=${pageSize}`)
      .then((result) => {
        // console.log(result.data); // Check API response in the console
        if (result.data.semesters) {
          setSemester(result.data.semesters); // Set batch data
  ...
  const handleEdit = (id) => {
    navigate(`/dashboard/semester/edit/${id}`);
  };
  ...
  const handleDelete = (id) => {
    if (
      ...
      api
        .delete(`/auth/semester/${id}`)
        .then((result) => {
          if (result.data) {
            setSemester(semester.filter((item) => item.id !== id)); // Remove deleted batch from state
  ...
  return (
    <div className="px-5 mt-3">
      <div className="d-flex justify-content-center">
        <h3 style={{ color: "red" }}>List of Semester Shown Below:-</h3>
      </div>
      {permissions.canAdd && (
        <Link to="/dashboard/semester/add" className="btn btn-success">
          Add Semester
        </Link>
      )}
        ...
        {isLoading ? (
          ...
          <TableContainer component={Paper}>
            <Table>
              ...
              <TableBody>
                {Array.isArray(semester) && semester.length > 0 ? (
                  semester.map((c) => (
                    <TableRow key={c.id}>
                      ...
                      {/* <TableCell><b>{c.pattern}</b></TableCell> */}
                      <TableCell>
                        {permissions.canEdit && (
                          <button
