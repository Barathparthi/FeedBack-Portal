...
const InstitutionView = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    api
      .get(`/auth/institution?page=${currentPage}&pageSize=${pageSize}`)
      .then((result) => {
        // console.log(result.data); // Check API response in the console
        if (result.data.institutions) {
          setInstitution(result.data.institutions); // Set batch data
  ...
  const handleEdit = (id) => {
    navigate(`/dashboard/institution/edit/${id}`);
  };
  ...
  const handleDelete = (id) => {
    if (
      window.confirm("Are you sure you want to delete this institution record?")
    ) {
      api
        .delete(`/auth/institution/${id}`)
        .then((result) => {
          if (result.data) {
            setInstitution(institution.filter((item) => item.id !== id)); // Remove deleted batch from state
  ...
  return (
    <div className="px-5 mt-3">
      <div className="d-flex justify-content-center">
        <h3 style={{ color: "red" }}>List of Institution Shown Below:</h3>
      </div>
      {permissions.canAdd && (
        <Link to="/dashboard/institution/add" className="btn btn-success">
          Add Institution
        </Link>
      )}
