...
const ProgramView = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    api
      .get(`/auth/program?page=${currentPage}&pageSize=${pageSize}`)
      .then((result) => {
        // console.log(result.data); // Check API response in the console
        if (result.data.programs) {
          setprogram(result.data.programs); // Set batch data
  ...
  const handleEdit = (id) => {
    navigate(`/dashboard/program/edit/${id}`);
  };
  ...
  const handleDelete = (id) => {
    if (
      window.confirm("Are you sure you want to delete this program record?")
    ) {
      api
        .delete(`/auth/program/${id}`)
        .then((result) => {
          if (result.data) {
            setprogram(program.filter((item) => item.id !== id)); // Remove deleted batch from state
  ...
  return (
    <div className="px-5 mt-3">
      <div className="d-flex justify-content-center">
        <h3 style={{ color: "red" }}>List of Program Shown Below:</h3>
      </div>
      {permissions.canAdd && (
        <Link to="/dashboard/program/add" className="btn btn-success">
          Add Program
        </Link>
      )}
