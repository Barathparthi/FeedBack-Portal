...
const SemesterType = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await api.post(`/auth/semtype`, formData);
      console.log('Form data submitted successfully:', response.data);
      setSuccessMsg('SemesterType registered successfully!');
      setTimeout(() => {
      ...
      if (error.response && error.response.status === 409) {
        ...
        setErrorMsg('Failed to register SemesterType. Please try again!');
      }
      setTimeout(clearMessages, 5000);  // Clear message after 10 seconds
    } finally {
      setLoader(false);
          ...
          <div className="d-flex justified-content-center align-items-center mt-3">
            ...
            >
              <Typography variant="h4" align="center">
                Semester - Type Details
              </Typography>
              <Typography variant="h6" color="text.primary" align="center">
                Please Enter Semester Type Details in the below Textfield!...
              </Typography>
              <Box
                component="form"
                noValidate
