...
const LovCategory = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await api.post(`/auth/lovcat`, formData);
      console.log('Form data submitted successfully:', response.data);
      setSuccessMsg('Lov registered successfully!');
      ...
      if (error.response && error.response.status === 409) {
        ...
        setErrorMsg('Failed to register Lov. Please try again!');
      }
      setTimeout(clearMessages, 5000);  // Clear message after 10 seconds
    } finally {
      setLoader(false);
          ...
          <div className="d-flex justified-content-center align-items-center mt-3">
            ...
            >
              <Typography variant="h4" align="center">
                Lov Category Details
              </Typography>
              <Typography variant="h6" color="text.primary" align="center">
                Please Enter Lov Category Details in the below Textfield!...
              </Typography>
              <Box
                component="form"
                noValidate
