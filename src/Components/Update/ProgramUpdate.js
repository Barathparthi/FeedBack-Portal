...
const ProgramUpdate = () => {
    ...
    api
      ...
      .then((result) => {
        if (result.data) {
          ...
          setErrorMsg("Error fetching program record.");
        }
      })
      .catch((err) => setErrorMsg("Error fetching program record."));
  }, [id]);
  ...
  const handleSubmit = (e) => {
    ...
    setLoader(true);
    api
      .put(`/auth/program/${id}`, program)
      .then((result) => {
        if (result.data) {
          setSuccessMsg("program entry updated successfully!");
          setTimeout(() => {
      ...
      .catch((err) => {
        setErrorMsg("Error updating program record.");
        setLoader(false);
      });
  };

  const handleCancel = () => {
    navigate("/dashboard/program"); // Navigate to /dashboard/program
  };
        ...
        <Grid item md={6} component={Paper} elevation={6} square>
          <Box sx={{ my: 8, mx: 4, display: "flex", flexDirection: "column" }}>
            ...
            <Typography variant="h6" color="text.primary" align="center">
              Please Enter program Details below:
            </Typography>
            <Box
              component="form"
              noValidate
...
<FormControl fullWidth margin="normal" required>
                ...
                <Select
                  ...
                  id="degree_id"
                  name="degree_id"
                  value={program.degree_id} // Correctly bind the value
                  label="degree"
                  onChange={handleChange}
                  fullWidth
                  MenuProps={{
